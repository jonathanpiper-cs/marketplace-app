...
const LocalizedDropdown = () => {
  ...
  useEffect(() => {
    ...
    ContentstackAppSDK.init().then(async (appSDK: any) => {
      ...
      try {
        ...
        console.log(localizationOptionsUID, entry.locale);
        await stack
          .ContentType(OPTIONS_UID)
          .Entry(localizationOptionsUID)
          .language(entry.locale)
          .fetch()
          .then((options: any) => {
            console.log(options.entry.localization_options);
  ...
  useEffect(() => {
    console.log(localizationOptions);
  }, [localizationOptions]);
